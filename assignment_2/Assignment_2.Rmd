...
data <- fread(here('Section1_4.dat'), 
                   sep = ' ', col.names = c('Time_To_Infection','Censored','Treatment'), 
                   colClasses = c('numeric', 'numeric', 'numeric'))
```
Below is a table summarising the survival function estimates, $\hat{S(t)}$, along with their standard errors, $\sigma$. As we are estimating the survival function, we employed the Kaplan-Meier method as it estimates the survival function directly (as opposed to the cumulative hazard rate, which would require a conversion)

For the surgical placement, we have:
...
For the percutaneous catheter placement:

```{r percutaneous, echo=FALSE}

print(kable(df_perc_head_tail))
```

We can see that the survival function is strictly decreasing, which is as expected.
...
print(ggsurv)
```
```{r hazard, echo=FALSE, warning=FALSE}
...
For the surgical technique's cumulative hazard rate estimate
```{r}
...
For the percutaneous technique's cumulative hazard rate estimate
```{r}
